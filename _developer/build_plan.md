...
| 9 | 2026-04-12 | General | `user_config.yaml` never created on disk. `DRY_RUN` and `ALPACA_PAPER` have no persistent write path — they were set as Replit secrets which have since been cleared, so the app reverted to the code default of `DRY_RUN=true`. All scheduler activity since then has been observation-only; no orders submitted despite live positions being held. **Fix**: create `user_config.yaml` at project root with `DRY_RUN=false` and `ALPACA_PAPER=false` as committed defaults. Optionally expose both as toggles on the Settings page so they can be changed without touching files or secrets. | — | OPEN |
| 10 | 2026-04-12 | 4/5 | Stop-loss and target-limit orders expire or are canceled within the same trading day, leaving all open positions unprotected overnight. Root cause: (a) separate SELL LIMIT (target) orders are submitted as day orders — they expire at market close and are never re-placed the next morning; (b) separate SELL STOP orders are explicitly canceled by the scheduler at the start of each assessment cycle before a potential re-buy, and are not re-placed when the position is already held. The intended design (bracket orders with linked stop and target legs) is not taking effect — all orders in history show `legs=0`. **Fix**: ensure protective orders are placed as GTC, and re-place them at the start of each assessment for positions already held if no open stop exists at the broker. Alternatively, make bracket order submission work correctly so the stop and target legs persist until the position is closed. | — | OPEN |
